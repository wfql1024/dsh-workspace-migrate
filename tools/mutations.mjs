...
export const MUTATIONS = [
  ...
  {
    name: 'backup-target-ignored',
    file: 'lib/dsh-workspace-migrate.mjs',
    suite: 'test/selftest.mjs',
    breaks: 'the "back up the destination and overwrite" flag is ignored, so a non-empty destination is refused',
    from: "  const backupExistingTarget = options['backup-target'] === true",
    to: '  const backupExistingTarget = false /* MUTATION: backup-target ignored */',
  },
  {
    name: 'live-backup-skipped',
    file: 'lib/live-move.mjs',
    suite: 'test/livetest.mjs',
    breaks: 'the live path moves into a non-empty destination without parking it first',
    from: 'const moved = moveProjectDirectory(pre.from, pre.to, { backupTarget: pre.project.backupTarget === true })',
    to: 'const moved = moveProjectDirectory(pre.from, pre.to) /* MUTATION: backup target ignored */',
  },
  {
