 ...
 *   without this the rollback fails and the project is left half-moved. A destination that
 *   holds anything at all is refused, whatever the flag says — the check reports it, and the
 *   user decides what to do with that directory.
...
export function moveProjectDirectory(from, to, { replaceEmptyDestination = false } = {}) {
  ...
  if (fs.existsSync(to)) {
    const occupied = !isEmptyDirectory(to)
    if (occupied || !replaceEmptyDestination) {
      return {
        ok: false,
        error: occupied
          ? `the destination already holds files: ${to} — empty it yourself, or choose "only change the path"`
          : `the destination already exists: ${to}`,
      }
    }
    try {
      fs.rmdirSync(to)
...
export async function inspectLiveMove(services, options) {
  ...
  if (project.willMove) {
    ...
    // Moving the project body into a directory that already holds files would merge two trees,
    // so the check fails and the user decides what to do with that directory. An empty
    // destination — or a missing one, which robocopy creates — is fine.
    if (project.destinationExists && project.destinationHasContent) {
      blockers.push(
        `the destination already holds files: ${to} — empty it yourself, or choose "only change the path" if the workspace should simply point there`,
      )
    }
