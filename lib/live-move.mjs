...
import { projectKey, relocateSessionLog, parseGenerationLogFilename } from './zstd-frames.mjs'
...
export async function relocateLiveSession({ services, session, sessionId, newCwd, sessionsRoot }) {
  ...
  // A session directory routinely holds SEVERAL generation logs: DSH writes a new generation
  // when it upgrades the log format and keeps the older file as history. Moving only the
  // generation the writer holds leaves the others under the old project key, and DSH then
  // refuses the session outright — "duplicate JSONL session id ... appears in multiple project
  // directories" — which it raises on the next `attachSession`. So every generation travels
  // together, each with its own header rewritten.
  ...
  // Step 2: no `await` from here until the writer has been retargeted. Every generation is
  // moved inside this stretch, and every move is synchronous.
  ...
  removeIfEmpty(fromDir)
  removeIfEmpty(dirname(fromDir))

  // If a generation log is still under the old key, the move left the session id in two
  // project directories and DSH will refuse the session from now on. Fail loudly here instead
  // of at some later `attachSession`, so the caller unwinds it.
  const stranded = fs.existsSync(fromDir) ? fs.readdirSync(fromDir).filter((name) => parseGenerationLogFilename(name) !== undefined) : []
  if (stranded.length > 0) {
    throw new Error(
      `the relocation left session log(s) behind under the old project key: ${stranded.join(', ')} in ${fromDir} — DSH refuses a session id that appears in two project directories`,
    )
  }
  ...
  return {
    ...
    bytes: moved.bytes,
    generations: [{ name: fileName, from: fromFile, to: toFile }, ...alsoMoved.map((entry) => ({ name: entry.name, from: join(fromDir, entry.name), to: join(toDir, entry.name) }))],
    writerRebound: true,
    liveHeaderUpdated,
  }
}
...
export function undoRelocateLiveSession(services, session, sessionId, relocation, sessionsRoot) {
  const persistence = services.get('sessionPersistence')
  const writer = liveWriterOf(persistence, sessionId)
  // Every generation moved forward, so every generation moves back.
  const moved = Array.isArray(relocation.generations)
    ? relocation.generations
    : [{ name: undefined, from: relocation.fromFile, to: relocation.toFile }]
  const restored = []
  for (const entry of moved) {
    restored.push(relocateSessionLog(entry.to, entry.from, relocation.previousCwd))
  }
  if (isRebindable(writer)) {
 ...
 *
 * Destination policy (D21), identical in both directions of a move:
 *   · missing            → the copy creates it;
 *   · exists but EMPTY   → taken over: the empty directory is removed first so the copy starts
 *                          clean. This is what "empty is acceptable" means for both the forward
 *                          move (a destination the user prepared) and the undo (`robocopy /MOVE`
 *                          leaves the original path behind as an empty husk);
 *   · anything at all inside (a folder counts) → refused, and nothing is touched.
 *
 * @returns `{ ok, mode }` or `{ ok: false, error, detail }`.
 */
export function moveProjectDirectory(from, to) {
  ...
  if (fs.existsSync(to)) {
    if (!isEmptyDirectory(to)) {
      return {
        ok: false,
        error: `the destination is not empty: ${to} — empty it yourself, or choose "only change the path"`,
      }
    }
    try {
      fs.rmdirSync(to)
...
/**
 * Signal markers for every user-facing line.
 *
 * A note written as flat prose reads as bad news: "the projection cache could not be
 * checkpointed" sounds like a failure even though it is handled, and "project directory
 * moved" is a success that looks like a log line. Every note therefore announces itself:
 *
 *   `[√]` something was done        `[!]` degraded, but handled
 *   `[×]` something failed          `[i]` neutral context
 *
 * The markers travel inside the strings, so the tool output, the browser dialog and the
 * report file all carry the same signal.
 */
export const MARK = { ok: '[√]', warn: '[!]', error: '[×]', info: '[i]' }

/** Whether a line already carries a signal marker (the client relies on this). */
export const MARKER_PATTERN = /^\[[√×!i]\]/
 ...
 * @param options - `{ fromPath, toPath, sessionIds?, moveProject?, projectOnly? }`.
 *   `projectOnly` answers just the questions that hold with DSH stopped too (does the source
 *   exist, is the destination usable) and skips the live-only preconditions — the workspace
 *   registry, its records and the session scan. The manual/stop-DSH flow asks this before
 *   writing a plan, so a registry that is unavailable cannot block a fallback that does not
 *   need it.
 */
export async function inspectLiveMove(services, options) {
  ...
  const noteWarn = (value) => notes.push(`${MARK.warn} ${value}`)
  const noteInfo = (value) => notes.push(`${MARK.info} ${value}`)
  ...
  const project = {
    ...
    destinationHasContent: typeof to === 'string' && fs.existsSync(to) && !isEmptyDirectory(to),
    destinationRootExists: false,
    createDestination: false,
  }
  if (typeof to === 'string' && to.length > 0) {
    // The drive must exist before anything can be created on it; the check names that up front
    // instead of failing halfway through the move.
    project.destinationRootExists = fs.existsSync(parse(to).root)
  }
  if (project.willMove) {
    if (!project.sourceExists) blockers.push(`the source project directory does not exist: ${from}`)
    // Moving the project body into a destination that is not empty would merge two trees, so the
    // check fails and the user decides what to do with that directory. "Empty" is the top-level
    // listing — a folder counts, even an empty one (D21). A missing destination is fine: the move
    // creates it.
    if (project.destinationExists && project.destinationHasContent) {
      blockers.push(
        `the destination is not empty: ${to} — empty it yourself, or choose "only change the path" if the workspace should simply point there`,
      )
    } else if (project.destinationExists) {
      noteOk('the destination exists and is empty, so the project can be moved into it')
    } else {
      noteInfo(`the destination directory does not exist yet; the move creates it: ${to}`)
    }
  } else if (!project.destinationExists) {
    // "Only change the path" registers a workspace at a directory that need not exist yet; the
    // move creates it (an empty directory is the only thing written). Reporting it here keeps the
    // user from being surprised, and a missing drive is still a real blocker.
    project.createDestination = true
    if (!project.destinationRootExists) {
      blockers.push(`the destination's drive does not exist, so it cannot be created: ${to}`)
    } else {
      noteInfo(`the destination directory does not exist yet; it will be created: ${to}`)
    }
  }

  if (projectOnly) {
    return { ok: blockers.length === 0, blockers, notes, source: null, sessionIds: [], liveIds: [], project, from, to, projectOnly }
  }
  ...
  if (targetExisting !== undefined) {
    noteInfo(`a workspace is already registered at ${to}; its registration will be reused, not duplicated`)
  }

  // DSH refuses to boot when two workspace records claim one path ("path ... is claimed by
  // both workspace A and B"), and this move cannot fix that: `registry.create` is idempotent
  // per path, so it would silently pick one of the two and leave the other in place. Refusing
  // here turns an unbootable store into a message naming the records to delete.
  for (const [label, claimed] of [
    ['source', from],
    ['destination', to],
  ]) {
    const claimants = entities.filter((entity) => pathEquals(entityPath(entity), claimed))
    if (claimants.length > 1) {
      blockers.push(
        `the ${label} path ${claimed} is claimed by ${claimants.length} workspace records (${claimants
          .map((entity) => String(attempt(() => entity.id, '?')))
          .join(', ')}) — DSH cannot boot from that; delete the extra one in the sidebar first`,
      )
    }
  }
  ...
  if (persistence !== undefined && typeof persistence.list === 'function') {
    try {
      ...
      noteWarn(`sessionPersistence.list() failed, so only the registry index was searched: ${messageOf(error)}`)
    }
  } else {
    noteWarn('the sessionPersistence service is unavailable, so only the registry index was searched')
  }

  const all = [...new Set([...indexed, ...discovered])]
  const unindexed = discovered.filter((id) => !indexed.includes(id))
  if (unindexed.length > 0) {
    noteInfo(`${unindexed.length} session(s) with this cwd are stored but not indexed on the workspace; they are included`)
  }
  ...
  if (liveIds.length > 0) {
    noteOk(`${liveIds.length} session(s) are running and will be relocated in process, without interrupting them`)
  }
  if (sessions === undefined || typeof sessions.get !== 'function') {
    noteWarn('the sessions service is unavailable, so the cold-session check could not be performed')
  }
  if (agents === undefined || typeof agents.get !== 'function') {
    noteWarn('the agents service is unavailable, so the live-agent check could not be performed')
  }
...
async function runLiveMove(services, options) {
  // `projectOnly` answers a narrower question than a real move needs, so it is forced off here:
  // this path must own the full precondition set (registry, records, sessions) whatever the caller
  // asked for when it merely inspected.
  const pre = await inspectLiveMove(services, Object.assign({}, options, { projectOnly: false }))
  if (!pre.ok) {
  ...
  // ── layer 0: the destination directory itself ─────────────────────────────
  //
  // Only "only change the path" needs this: the workspace must point at a real directory and
  // `registry.create()` realpaths its argument, so a destination that does not exist yet is
  // created here — an empty directory is the only thing this action writes.
  if (pre.project.createDestination === true && !fs.existsSync(pre.to)) {
    try {
      fs.mkdirSync(pre.to, { recursive: true })
    } catch (error) {
      return {
        ok: false,
        stage: 'destination-directory',
        blockers: [`could not create the destination directory ${pre.to}: ${messageOf(error)}`],
        reportFile,
        notes,
      }
    }
    undo.push({
      ...
      // Only removes it while it is still empty: anything written there in the meantime is not
      // ours to delete.
      run: () => removeIfEmpty(pre.to),
    })
    noteOk(`created the destination directory: ${pre.to}`)
  }
  ...
  if (pre.project.willMove) {
    ...
    if (!moved.ok) {
      ...
      return { ok: false, stage: 'project-directory', blockers: [moved.error], detail: moved.detail, undoErrors, reportFile, notes }
    }
    undo.push({
      name: 'project-directory',
      run: () => {
        // The forward move leaves the original path as an empty directory on Windows; an empty
        // destination is always taken over, so the undo needs no special flag (D21).
        const back = moveProjectDirectory(pre.to, pre.from)
        if (!back.ok) throw new Error(back.error)
      },
    })
    noteOk(`project directory moved (${moved.mode}): ${pre.from} -> ${pre.to}`)
  }
  ...
  if (target === undefined) {
    ...
    noteInfo(`reusing the workspace already registered at ${pre.to}`)
  }
  ...
  if (coldIds.length > 0) {
    ...
    noteOk('every session is running, so the spawned engine was not used at all')
  }
  ...
  if (projectionCache !== undefined && typeof projectionCache.write === 'function') {
    for (const done of liveRelocations) {
      ...
      if (written === undefined) noteWarn(`the projection cache could not be checkpointed for ${done.id}; it self-heals on the next write`)
    }
  }
  ...
  if (headerIndex !== undefined && typeof headerIndex.get === 'function' && typeof headerIndex.delete === 'function') {
    ...
    noteWarn('the registry exposes no header index to invalidate; validation will read from disk')
  }
  ...
  return {
    ...
    destinationCreated: pre.project.createDestination === true,
    sessionIds,
