...
/**
 * Where a displaced destination tree is parked: the user's Desktop.
 *
 * Same rule as the engine's copy (`lib/dsh-workspace-migrate.mjs`): the Windows Desktop can be
 * redirected into OneDrive or keep a localized name, so pick the first candidate that exists.
 */
function desktopRoot() {
  const home = process.env.USERPROFILE || process.env.HOME || homedir()
  const candidates = [join(home, 'Desktop'), join(home, 'OneDrive', 'Desktop'), join(home, '桌面')]
...
function desktopBackupDir(label) {
  ...
  return join(desktopRoot(), `dsh-workspace-migrate-backup-${stamp}-${slug}`)
}
 ...
 */
export function moveProjectDirectory(from, to, { replaceEmptyDestination = false, backupTarget = false } = {}) {
  ...
  if (fs.existsSync(to)) {
    ...
    if (occupied && backupTarget) {
      // The user asked for the destination to be kept and overwritten: park its content on the
      // Desktop (a move, so nothing is duplicated and the destination is clean for the copy).
      const backup = desktopBackupDir(basename(to) || 'workspace')
      try {
        fs.mkdirSync(dirname(backup), { recursive: true })
        fs.renameSync(to, backup)
        parked = backup
      } catch (error) {
        return { ok: false, error: `could not park the existing destination at ${backup}: ${messageOf(error)}` }
      }
    } else if (occupied || !replaceEmptyDestination) {
      return {
        ok: false,
        error: occupied
          ? `the destination already holds files: ${to} — turn on "back up the destination and overwrite", or empty it yourself first`
  ...
  if (process.platform === 'win32') {
    ...
    if (result.error !== undefined && result.error !== null) {
      return { ok: false, error: `robocopy could not start: ${messageOf(result.error)}`, parked }
    }
    const code = result.status ?? -1
    if (code < 0 || code >= 8) {
      return { ok: false, error: `robocopy failed with exit code ${code}`, detail: `${result.stdout ?? ''}`.slice(-2000), parked }
    }
    ...
    removeIfEmpty(from)
    return { ok: true, mode: 'robocopy /E /MOVE', parked }
  }

  try {
    fs.cpSync(from, to, { recursive: true })
    fs.rmSync(from, { recursive: true, force: true })
    return { ok: true, mode: 'cp + rm', parked }
  } catch (error) {
    return { ok: false, error: messageOf(error), parked }
  }
}
...
export async function inspectLiveMove(services, options) {
  ...
  const project = {
    ...
    backupTarget: options.backupTarget === true,
  ...
  if (project.willMove) {
    ...
    if (project.destinationExists && project.destinationHasContent && !project.backupTarget) {
      blockers.push(
        `the destination already holds files: ${to} — turn on "back up the destination and overwrite" (its content goes to your Desktop), or empty it yourself first`,
      )
    }
    if (project.destinationExists && project.destinationHasContent && project.backupTarget) {
      notes.push(`${MARK.info} the destination's current files will be parked on your Desktop before the project is moved in`)
    }
  }
...
async function runLiveMove(services, options) {
  ...
  if (pre.project.willMove) {
    const moved = moveProjectDirectory(pre.from, pre.to, { backupTarget: pre.project.backupTarget === true })
    if (!moved.ok) {
      return { ok: false, stage: 'project-directory', blockers: [moved.error], detail: moved.detail, reportFile, notes }
    }
    const parked = moved.parked === null || moved.parked === undefined ? null : moved.parked
    undo.push({
      ...
      run: () => {
        ...
        // Restore the destination's own files too: they were parked on purpose, and leaving them
        // on the Desktop after a rollback would be a half-undo.
        if (parked !== null && fs.existsSync(parked)) {
          if (!fs.existsSync(pre.to) || isEmptyDirectory(pre.to)) {
            fs.mkdirSync(dirname(pre.to), { recursive: true })
            fs.renameSync(parked, pre.to)
          }
        }
      },
    })
    ...
    if (parked !== null) {
      noteWarn(`the destination's previous files were parked at ${parked} (put them back or delete them yourself)`)
    }
  }
