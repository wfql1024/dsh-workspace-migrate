...
export function moveProjectDirectory(from, to) {
  ...
  if (fs.existsSync(to)) return { ok: false, error: `the destination already exists: ${to}` }
  ...
  if (coldIds.length > 0) {
    ...
    if (!(engine.ok === true && engine.json !== undefined && engine.json.status === 'ok')) {
      ...
      return {
        ok: false,
        stage: 'file-layer',
        blockers: [engine.error ?? engine.json?.error ?? 'the engine refused the relocation'],
        engineReport: engine.json,
        engineDetail: engine.json?.errors,
        reportFile,
