...
function buildPlan(options) {
  ...
  if (!path.isAbsolute(to)) {
    errors.push(`the destination must be an absolute path: ${to}`)
  }
  // "Move the files too" into a destination that is not empty would silently merge two trees, so
  // the check fails there and the user decides what to do with that directory. Empty means the
  // top level holds NOTHING — a directory counts as content even when it is itself empty, because
  // some projects use a bare folder as a marker (and merging folders is how trees get tangled).
  // A missing destination is fine: robocopy creates it.
  const destinationEntries = newExists ? readEntries(to) : []
  const destinationHasContent = destinationEntries.length > 0
  if (projectAction === 'move' && destinationHasContent) {
    errors.push(
      `the destination directory is not empty (${describeEntries(destinationEntries)}): ${to} — empty it yourself, or choose "only change the path" if the workspace should simply point there`,
    )
  }
  // "Only change the path" registers a workspace at a directory that need not exist yet; apply
  // creates it (nothing else is written), so this is a note rather than a check failure.
  const createDestination = projectAction === 'keep' && !newExists
  if (createDestination) {
    if (fs.existsSync(path.parse(to).root)) {
      warnings.push(`the destination directory does not exist yet; apply creates it: ${to}`)
    } else {
      errors.push(`the destination's drive does not exist, so it cannot be created: ${to}`)
    }
  } else if (projectAction !== 'keep' && !newExists) {
    warnings.push(`the destination directory does not exist yet; the move creates it: ${to}`)
  }
...
/**
 * The TOP LEVEL of a directory, one entry per name — no recursion.
 *
 * This is what "is the destination empty?" means everywhere (D21): anything at the top level counts,
 * including a directory that is itself empty. `directoryStats().files` is NOT the same question — it
 * counts files recursively, so a destination holding only folders reports `files: 0` and would be
 * merged into. Unreadable/missing paths report `[]`.
 */
function readEntries(dir) {
  try {
    return fs.readdirSync(dir, { withFileTypes: true }).map((entry) => ({ name: entry.name, directory: entry.isDirectory() }))
  } catch {
    return []
  }
}

/** Name the entries in a refusal message: `3 item(s): 2 file(s), 1 folder(s) — "a", "b", "c"`. */
function describeEntries(entries) {
  ...
  if (directories > 0) parts.push(`${directories} folder(s)`)
  const names = entries.slice(0, 8).map((entry) => `"${entry.name}"${entry.directory ? '/' : ''}`)
  const more = entries.length > names.length ? `, +${entries.length - names.length} more` : ''
  return `${entries.length} item(s)${parts.length > 0 ? `: ${parts.join(', ')}` : ''} — ${names.join(', ')}${more}`
}
...
function applyPlan(plan, options, report) {
  ...
  record('project', () => {
    if (projectAction === 'keep') {
      // "Only change the path": the workspace must point at a real directory, so a destination that
      // does not exist yet is created here. This is the only write this action performs.
      let created = false
      if (!fs.existsSync(plan.to)) {
        fs.mkdirSync(plan.to, { recursive: true })
        created = true
      } else if (!fs.statSync(plan.to).isDirectory()) {
        throw new Error(`the destination exists but is not a directory: ${plan.to}`)
      }
      return { action: 'keep', created }
    }
    if (!fs.existsSync(plan.from)) throw new Error(`source project directory vanished: ${plan.from}`)
    // A destination that is not empty would be merged with the project body, so refuse. The re-check
    // is here rather than trusting planning: the directory can gain entries in between. What counts
    // as empty is the top-level listing, folders included (D21).
    if (projectAction === 'move') {
      const entries = readEntries(plan.to)
      if (entries.length > 0) {
        throw new Error(
          `the destination directory is not empty (${describeEntries(entries)}), so the project will not be moved into it: ${plan.to}`,
        )
      }
    }
...
function verifyPlan(plan, { expectMigrated = true, autoDetected = false, detectedState = undefined } = {}) {
  ...
  // project directory. "move"/"copy" always leave the source and the destination in play, so the
  // expected side is checked either way. "keep" writes nothing but the destination directory
  // itself, which only exists once apply has created it — so the expectation is one-directional:
  // after apply it must be there, while the original state says nothing about it (a rollback that
  // runs before the project body is moved back would otherwise fail on a missing destination).
  if (expectMigrated || plan.project.action !== 'keep') {
...
function humanSummary(plan) {
  ...
  lines.push(`  project     : ${plan.project.action} (source ${plan.project.oldExists ? 'present' : 'absent'}, destination ${plan.project.newExists ? 'present' : 'absent'}${plan.project.createDestination ? ', will be created' : ''}${plan.project.oldFileCount !== undefined ? `, ${plan.project.oldFileCount} files` : ''})`)
