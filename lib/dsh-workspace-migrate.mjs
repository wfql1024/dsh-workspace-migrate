...
function buildPlan(options) {
  ...
  // "Move the files too" into a destination that already holds files would silently merge two
  // trees, so the check fails there and the user decides what to do with that directory. A missing
  // destination (robocopy creates it) and an empty one are both fine.
  const destinationHasContent = newExists && (newStat?.files ?? 0) > 0
  if (projectAction === 'move' && destinationHasContent) {
    errors.push(
      `the destination directory already holds ${newStat?.files} item(s): ${to} — empty it yourself, or choose "only change the path" if the workspace should simply point there`,
    )
  }
...
function applyPlan(plan, options, report) {
  ...
  record('project', () => {
    ...
    // A destination that holds files would be merged with the project body, so refuse. The re-check
    // is here rather than trusting planning: the directory can gain files in between.
    if (projectAction === 'move' && fs.existsSync(plan.to) && (directoryStats(plan.to).files ?? 0) > 0) {
      throw new Error(
        `the destination directory already holds files, so the project will not be moved into it: ${plan.to}`,
      )
    }
