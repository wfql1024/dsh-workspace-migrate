...
function probeDshProcesses() {
  if (process.platform !== 'win32') {
    ...
    const matches = r.stdout
      .split('\n')
      .filter((line) => /@deepseek-ai[\\/]dsh|dsh[\\/]lib|dsh-cli/.test(line))
  ...
  const script = [
    ...
    `Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match '@deepseek-ai[\\\\/]dsh|dsh[\\\\/]lib|dsh-cli' -and $_.ProcessId -ne ${process.pid} -and $_.CommandLine -notmatch 'dsh-workspace-migrate' } | Select-Object -ExpandProperty CommandLine`,
  ].join('; ')
  const r = spawnSync(
    'powershell.exe',
...
function buildPlan(options) {
  ...
  if (projectAction === 'move' && newExists) {
    errors.push(`destination project directory already exists: ${to} — use --project copy, --project keep, or remove it`)
  }
  if (projectAction === 'move' && oldExists && newExists) {
    errors.push(`both source and destination project directories exist: ${from} / ${to}`)
  }
  ...
  const plan = {
    ...
    running: { dshProcesses: dshProcesses.matches, checked: dshProcesses.checked, locks },
    project: {
...
function stageRun(plan) {
  ...
  const guard = [
    ...
    `"%NODE_EXE%" "%~dp0dsh-workspace-migrate.mjs" check-quiescent`,
    'if errorlevel 1 (',
    '  echo.',
...
function preflightForApply(plan, options, { recheckDrift = true } = {}) {
  ...
  if (options['allow-running'] !== true) {
    ...
    } else if (!processes.checked) {
      // An answer the probe could not produce is not permission to rewrite the store.
      blockers.push(
        'could not determine whether DSH is running: the process probe failed. Confirm DSH is stopped yourself, then re-run with --allow-running',
      )
    }
  }
...
function applyPlan(plan, options, report) {
  ...
  record('project', () => {
    ...
    if (fs.existsSync(plan.to) && projectAction === 'move') {
      throw new Error(`destination project directory appeared since planning: ${plan.to}`)
    }
...
function main() {
  ...
  try {
    ...
    if (command === 'check-quiescent') {
      ...
      if (processes.matches.length > 0) {
        process.stderr.write(
          [
            ...
            ...processes.matches.map((line) => `  ${line.slice(0, 160)}`),
      ...
      if (!processes.checked) {
        process.stderr.write(
          `${TOOL} — could not determine whether DSH is running (the process probe failed).\n  Confirm DSH is stopped, then run the migration with --allow-running.\n`,
        )
        return 1
      }
      process.stdout.write('DSH is not running; safe to continue.\n')
      return 0
    }
