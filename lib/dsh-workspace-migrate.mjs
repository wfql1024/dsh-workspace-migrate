...
function stageRun(plan) {
  ...
  fs.writeFileSync(files.applyCmd, cmd(`apply --plan "%~dp0plan.json" --yes`))
  fs.writeFileSync(files.verifyCmd, cmd(`verify --plan "%~dp0plan.json"`))
  fs.writeFileSync(files.rollbackCmd, cmd(`rollback --plan "%~dp0plan.json" --yes`))
...
function preflightForApply(plan, options, { recheckDrift = true } = {}) {
  ...
  if (processes.matches.length > 0 && options['allow-running'] !== true) {
    blockers.push(
...
function verifyPlan(plan, { expectMigrated = true, autoDetected = false, detectedState = undefined } = {}) {
  ...
  add(
    ...
    duplicated.length === 0,
    duplicated.length === 0 ? `${byId.size} session directories scanned` : JSON.stringify(duplicated),
  )
