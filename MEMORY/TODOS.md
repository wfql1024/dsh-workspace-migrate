...
- [ ] **「从」只能选已注册的工作区**（D19 把它设成只读）。想迁一个"还没注册但确实存在"的目录时，得先在 DSH 里把它加成工作区。
- [ ] **「仅修改目录」必须给出一个合法路径**（D20 会自动创建它）。UNC/盘符不存在的路径会在 `Check` 阶段被拒。
- [ ] **「连同文件迁移」遇到"目标目录不是空的"要让用户自己清空**（D20+D21）。插件不代劳搬走/覆盖/备份，
      而且**空文件夹也算内容**，所以想借一个空的子目录暗示"合并到这里"是做不到的。
- [ ] **手动迁移的检查只看"停机也成立"的规则**（D21 的 `projectOnly`）。实时条件（注册表、运行中会话、
      多条记录声明同一路径）不会在生成计划前提示 —— 那些问题要等 `apply` 时才会以 blocker 的形式出现，
      或者由 live 路径的完整检查先发现。
- [ ] 源工作区只是被改名、或暂时没注册时，对应的暂存记录会被标成「无法使用」（D18）。这类计划本来也指向不存在的源。
...
- [ ] 界面英文提示与中文标签混排（note 是英文、行标题是中文）。想让非中文用户舒服需要统一，
      但那是一次文案大改，先记账。
- [ ] 侧栏窄条里的图标用的是自带字形 `⇄`，不是 DSH 官方图标集。想更统一可以改引
      `@deepseek-ai/dsh-client-ui-primitives`（`dsh-session-manager` 就是这么做的），
      代价是多一个**外部依赖**（多一个随 DSH 版本变动的面）。目前的取舍见 D22。
- [ ] 若 DSH 把 `sidebar.footer.action` 的 owner prop `wide` 改名/去掉，窄条会**静默退回带标签形态**
      （难看但不会坏）。升级 DSH 后按 FACTS §9 的三步只读核对能立刻看出来。
