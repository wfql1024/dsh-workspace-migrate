...
> **注意：本条的后半部分（复选框 + 桌面备份）已被 [D20](#d20-目标目录的创建与内容判定) 取代。**
> 下面保留的是当时的设计与理由。
...
> 由用户 2026-09-25 的两条要求产生：①带文件迁移时**去掉**「自动备份目标并覆盖」与桌面备份逻辑，
> 目标有内容就让 `Check` 失败并把原因写在 `Check` 输出里；②「仅修改目录」时目标路径不存在 → **默认自动创建**（只要路径合法）。

- **背景**：D17 曾让插件把目标目录里已有的内容**搬到桌面**再迁入（勾选「自动备份目标并覆盖」）。
  用户直接否掉了这条：插件不该动目标目录里的任何东西，也不该悄悄把用户的文件挪到别处；
  而"目标目录不存在"在「仅修改目录」下原本是硬错误（`registry.create()` 会 realpath 它的参数），
  用户希望它被自动创建。
- **决定**：
  - **删除**桌面备份：`desktopRoot` / `desktopBackupDir` / `--backup-target` / 面板复选框 / 回滚里的"搬回" 全部移除。
    目标目录的内容**在任何情况下都不会被移动、覆盖或备份**。
  - 「连同文件迁移」+ 目标有内容 → `Check` 失败（`inspectLiveMove` 的 blocker / `buildPlan` 的 error），
    文案同时给出两条出路（自己清空、改用「仅修改目录」）。
  - 「仅修改目录」+ 目标不存在 → `Check` 通过，并给一条 `[i]` 说明"执行时会创建"；
    执行时 **live 路径在 layer 0**、**引擎 apply 在 project 步骤**各建一次空目录（`recursive`，父目录一并创建）。
    盘符不存在 → 仍然 `Check` 失败（`path.parse(to).root` 不存在），因为那不是"合法的路径"。
  - 回滚：live 的 layer 0 压入 `destination-directory` 撤销项（只在目录仍为空时删掉它）；
    引擎 apply 的"创建"由 `plan.project.createDestination` 记录，verify 在 `expectMigrated` 时断言目标存在。
- **理由**：只读检查不写盘，所以"创建目录"必须发生在执行阶段；把目标内容当错误而不是当"可以搬走的东西"，
  是让插件的手伸得最短的做法。
- **代价**：`verify --expect original` 对 `keep` 计划不再断言目标目录存在（回滚可能先于"项目搬回"发生，
  那时目标目录还不存在 —— 见 DEV_LOGS 2026-09-25）。

## D21 "目标目录是空的" = 顶层没有任何条目；手动迁移也先 Check

> 用户实测报回两条（2026-09-25 下午）：①带文件迁移 + **手动**迁移时，目标目录下**只有文件夹**却被判通过
> （**太宽松**）；②带文件迁移 + **自动**迁移时，目标目录**已存在但为空**却被拒（**太严格**）。
> 用户给出的统一标准：不存在 → 自动创建后迁移；存在且内部没有任何文件夹和文件 → 可以迁移；
> 存在且内部已有文件夹或文件 → 不允许。

- **背景（两个不同的根因）**：
  - 宽松来自 D20 的实现：引擎用 `directoryStats(to).files > 0` 判"有没有内容"，而它**递归数的是文件**，
    所以一个只放着空文件夹的目标目录报告 `files: 0` ⇒ 计划放行、`apply` 的二次校验也放行 ⇒ 两棵树被合并。
  - 严格来自 `moveProjectDirectory`：它只在 `replaceEmptyDestination: true`（当时只有撤销会用）时才接管
    "已存在"的目标，于是**检查通过、迁移却在项目层失败**（`the destination already exists`）——
    用户看到的正是"自动迁移太严格"。
- **决定**：
  - **"空"只有一个含义**：目标目录**第一层没有任何条目**（文件夹、空文件夹、文件都算内容）。
    引擎里由 `readEntries(dir)`（一次 `readdirSync`，不递归）+ `describeEntries(entries)` 实现，
    `buildPlan` 与 `applyPlan` 的二次校验、live 的 `isEmptyDirectory` 用的都是这一条。
  - 「连同文件迁移」的三条规则（= 用户给的统一标准）：不存在 → 搬迁时创建；存在且为空 → 直接用；
    存在且非空 → `Check` 失败。失败文案给出**目录、项数、条目名**（`3 item(s): 2 file(s), 1 folder(s) — "a", "b/"`）
    以及两条出路。
  - `moveProjectDirectory(from, to)` **对两个方向都**统一：目标是空目录 → 删掉它让复制从干净状态开始；
    非空 → 拒绝。`replaceEmptyDestination` 这个只为撤销存在的开关**删除**（撤销同样是"目标是空壳"）。
  - **手动迁移先 Check**：点「生成计划」先调 `/live-inspect`，并且只问"停机时也成立"的问题
    （`projectOnly: true`：源路径、目标目录），失败就显示 `Check` 行且**不生成计划**。
    跳过的是实时前置条件（注册表、多条记录声明、运行中会话）—— 兜底路线不该被它自己不需要的东西挡住。
  - 引擎侧对 `move + 目标不存在` 补一条 warning（"搬迁时会创建"），与 `keep` 的提示对称。
- **理由**：一个判定只能有一个定义。"有没有文件"和"目录空不空"是两个问题，用前者回答后者就会合并目录树；
  而"检查说可以、执行却拒绝"比两边都严格更糟——用户无法从 `Check` 行推断结果。手动的 `projectOnly`
  则是让"同一套目标规则"覆盖两条路径，同时不把实时条件强加给兜底路线。
- **代价**：`projectOnly` 让 `/live-inspect` 有了两种语义，调用方必须显式选择（`runLiveMove` 内部强制
...
| 「目标目录还不存在，帮我把项目目录一起搬过去」复选框 | 把"要不要搬文件"和"目标目录存不存在"绑在一起，还要用户自己去确认目录状态 | D17（「连同文件迁移 / 仅修改目录」滑块） |
| 复选框「自动备份目标并覆盖」+ 把目标目录里已有的内容**搬到桌面**（D17 原设计） | 用户明确否掉：插件不该动目标目录里的东西，更不该把用户的文件挪到别处；目标有内容直接让 `Check` 失败并说明，用户自己决定怎么处理。「仅修改目录」下目标不存在也不再是硬错误，而是自动创建 | D20（目标内容 ⇒ Check 失败；`keep` 自动建目录；桌面备份逻辑整体删除） |
| 用 `directoryStats(to).files > 0` 判"目标目录有没有内容"（D20 的实现） | 它**递归数文件**：只放空文件夹的目标报告 `files: 0`，计划放行并合并两棵树（用户实测报回）。"有没有文件"不能回答"目录空不空" | D21（`readEntries` 只看顶层条目，文件夹也算内容） |
| `moveProjectDirectory` 只在撤销时接管"已存在"的目标（`replaceEmptyDestination`） | 正反两个方向语义不一致：检查放行了空目标，搬迁却在项目层报 `the destination already exists`（用户实测报回） | D21（空目标一律接管，开关删除） |
