...
### 2026-09-25 晚（追加）— README 改双语，并修掉两处过期内容

用户要求把 README 弄成双语后即可推送。做法：**单文件双语**（`README.md` 上半是中文、下半是英文，
顶部一行 `[中文](#中文) · [English](#english)` 互相跳转），这样从 GitHub 打开一个链接就能读到两种语言，
不必再点第二个文件。两侧章节一一对应（功能/UI 入口/用法/安全说明/兼容性/开发/文档/致谢/许可）。
...
### 2026-09-25 深夜（追加）— README 按上游同类插件的结构精简，细节移入 `MEMORY/USAGE*.md`

用户第二次数落 README"太长太啰嗦"，给了参照物（`dsh-session-manager` 的 `README.zh.md`），
并明确要求：**从 README 里精简掉的内容必须有别处存放**。

**做法**：
- README 采用参照物的骨架：`0 简介 / 1 功能（1.1–1.4）/ 2 UI入口 / 3 用法 / 4 安装 / 5 安全说明 /
  6 兼容性 / 7 开发 / 8 致谢 / 9 开源许可`，条目压成"一行一条、加粗起头"，
  中文 239 行 → **87 行**，英文 285 行 → **87 行**（两侧结构一一对应）。
- 删掉的细节**一个字都没丢**，全部搬进两份新文档：
  [`USAGE.md`](USAGE.md)（161 行，中文）与 [`USAGE.en.md`](USAGE.en.md)（195 行，英文）——
  三种 shell 的完整安装命令、启动日志 `10/10` 原文、排错表（9 行）、用法全流程（含三条目标目录规则全文）、
  安全与行为说明全文（10 条）、报告与备份的落盘位置、兼容性核对清单、开发/测试基线与变异数量、开发者记忆索引。
  README 在 4.2 / 5 / 6 / 7 四处各自链到对应小节（`USAGE.md#…` / `USAGE.en.md#…`）。
- `MEMORY.md` 的索引表新增这两份文件，并在"和代码/测试的关系"里补了一条约定：
  **README 只留精简版，删掉的细节一律进 `USAGE*.md`，不许"精简"成丢失**。

**验证**：写了一段逐条探针，拿 `git show HEAD:README.md` / `README.en.md` 里的老内容去新文件里找 ——
三种 shell 命令、`cmd /c rmdir` 排错行、`mounted — 10/10 routes`、`Ctrl+Shift+R`、两条 `.cmd` 的双信号说明、
`projectOnly: true`、`migration-backups/<时间戳>/`、`整文件解压 → 改 → 重压`、`574 项断言` 等全部命中；
唯一"找不到"的是英文排错小节的**标题原文** `If it will not install`（内容仍在，标题改成了 `## 2 Troubleshooting`）。

`npm pack --dry-run` 确认 `MEMORY/` 是整目录发布，两份新文档随包一起走（21 个文件）。
