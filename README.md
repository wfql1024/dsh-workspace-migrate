...
2. 选「方式」：
   - **连同文件迁移**（默认）：项目文件也搬到新路径。目标目录**不存在**（自动创建，合法路径即可）或
     **存在但为空**都没问题；**已经有内容**则 `Check` 直接失败，并在 `Check` 行写明是哪个目录、有多少项，
     提醒你**先自己清空**，或者改用「仅修改目录」——插件**不会**动目标目录里的任何东西，也不会把它搬到别处。
...
| 报"目标目录已经有文件" | 目标里已有内容。**先自己清空它**，或改用「仅修改目录」只把工作区指过去 |
...
- **目标目录的处理**（「连同文件迁移」）：不存在 → robocopy 直接创建；存在但为空 → 直接用；
  **存在且有内容** → `Check` 失败并写明目录和项数，提示你自己清空或改用「仅修改目录」。
  插件**绝不**移动、覆盖或备份目标目录里的东西。
...
- **会拒绝而不是硬来**：目标路径已被另一条**有会话的**工作区记录声明、源/目标路径被多条记录声明、
  拿不到可重定向的写句柄、**「连同文件迁移」但目标目录已有内容**、目标路径不在任何存在的盘上、源路径不存在
  —— 都会拒绝并说明原因。
