...
- **三种落地方式**：「连同文件迁移」（默认，把项目文件也搬过去）、「仅修改目录」（只把工作区指向你已经准备好的路径）、
  以及勾选「自动备份目标并覆盖」后**把目标目录里已有的内容先搬到桌面**再迁入。
...
2. 选「方式」：
   - **连同文件迁移**（默认）：项目文件也搬到新路径。目标目录里**已经有东西**时，必须同时勾选
     **「自动备份目标并覆盖」**——它会把目标目录的内容先搬到**桌面**（`dsh-workspace-migrate-backup-<时间戳>-<名字>`），
     再把项目迁进去；不勾选就直接失败并说明原因，让你自己处理。
   - **仅修改目录**：不动任何文件，只把工作区指向目标路径（该路径要已存在，或用它已有的注册记录）。
...
| 报"目标目录已经有文件" | 目标里已有内容。勾选「自动备份目标并覆盖」（内容会搬到桌面），或自己先清空 |
...
- **目标目录的处理**：不存在 → 直接创建；存在但为空 → 直接用；**存在且有内容** → 必须勾选「自动备份目标并覆盖」，
  否则拒绝。勾选后目标内容会被**移动到桌面**（`dsh-workspace-migrate-backup-<时间戳>-<名字>`），
  迁移失败回滚时它会**搬回原位**。
...
- **会拒绝而不是硬来**：目标路径已被另一条**有会话的**工作区记录声明、源/目标路径被多条记录声明、
  拿不到可重定向的写句柄、目标目录有内容但没勾选备份覆盖、源路径不存在 —— 都会拒绝并说明原因。
