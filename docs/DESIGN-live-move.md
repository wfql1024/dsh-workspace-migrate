...
- **勾选「手动迁移」**（原「停机计划」模式开关）后，同一个按钮变成「计划」，只生成脚本。
...
- 「打开目录」走宿主新增的 `/open-directory` 路由，**只允许打开 `<DSH_HOME>/migration-runs` 下的目录**
  （浏览器不能让宿主去开用户机器上任意路径）；`explorer.exe` 成功时也会返回非零退出码，所以只等
  `spawn`/`error` 事件、不看退出码。测试通过 `DSH_WORKSPACE_MIGRATE_DRY_OPEN=1` 让这一步只报告不启动。
